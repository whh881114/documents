...
## 听力复盘维护流程（2026-07-19 起生效）

- 每篇复盘保存在对应课题目录的 `review.md` 中；文件存在即表示该课题“已复盘”。
- 用户负责填写或修改：复盘日期、正确题数，以及具体的“错题复盘”记录。代理不得擅自覆盖用户已经填写的答案、错误原因或个人判断。
- 代理负责补充：`文章内容总结（chatgpt）`、`生词与表达（chatgpt）`，以及基于用户描述整理的总体问题分析和可执行的下次策略。
- 文章总结应反映原文的结构、主要内容和逻辑顺序；生词与表达应结合原文语境说明含义、用法和必要的例句，优先整理能帮助听力定位和理解逻辑的表达。
- 用户提供做题问题时，应把问题转化为具体策略，例如审题、关键词选择、答案形式预判、结构路标识别和错题止损；不得虚构用户未提供的具体答案或考试表现。
- 修改、新增或移动任何听力 `review.md` 后，必须在仓库 `雅思` 目录运行：

  ```powershell
  powershell -ExecutionPolicy Bypass -File .\00-web\scripts\Build-ListeningCatalog.ps1
  ```

- 生成后必须核验 `00-web/assets/data/listening-catalog.js` 与 `00-web/assets/data/listening-review-data.js` 已包含最新课题、复盘状态和复盘正文，并检查网页数据与 `review.md` 一致。
- 完成听力资料或听力网页的修改并通过同步与核验后，默认只暂存本次相关文件，执行 Git commit 并 push 到当前远程分支，不再等待用户逐次提醒；提交前必须检查工作区，不得带入无关改动。若用户当次明确要求暂不提交或暂不推送，则以用户当次要求为准。

## 当前目录命名规则（2026-07-18 起生效）

- 第一层目录固定为 `Part1`、`Part2`、`Part3`、`Part4`，`Part` 与数字之间不留空格。
- 每个 Part 下的主题分类按所含 `script.md` 数量从多到少排列，并从 `01` 开始连续编号；数量相同时保留原有顺序。
