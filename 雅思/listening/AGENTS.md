...
- 精听页显示的句子总数与提取结果一致。
- 第一条和最后一条均已提取。
- 页面句子条目顺序必须连续，没有重复或遗漏；只有条目明确出现说话人姓名时才显示其来源序号。
- 每条至少包含一段英文内容。
- 没有混入中文翻译、按钮文字、快捷键说明或音频控件文字。
- 文件中的来源名称和精听页 URL 正确。

如果页面显示 40 句，最终文件也必须保留 40 个 CSS Grid 条目。没有说话人姓名的续句仍需保留，但其编号列和说话人列留空。不能因为最后一句不完整而删除它。
...
每个来源句子条目使用一个无边框 CSS Grid。固定分成三列：编号、说话人、正文。只有条目明确出现说话人姓名时才在编号列显示来源序号；没有姓名的续句，其编号列和说话人列都留空。
